---
subtitle: "Plan d'analyse statistique (V2)"
...
Un tableau présentant la consommation de médicaments dans chaque groupe par classe thérapeutique & par mode d'administration (en particulier pour les médicaments inhalés) sera réalisé. Des tests pourront être réalisés si le nombre de classes est faible.
...
*Les différents aspects de la motivation seront évalués à l'aide du questionnaire validé \textsc{BREQ-2}.*

Les réponses (échelles de Likert) seront tout d'abord présentées  item par item (package `ggstats`[@ggstats]) sans compararaison afin de ne pas multiplier les tests puis par score : 

- Score de motivation autodeterminée
- Score de motivation non autodeterminéeé
- Score global de motivation (SDI)

Pour chacun de ces scores (résultat numérique) une comparaison entre les deux groupes sera réalisée par un test t de \textsc{Student} & présentée par un tableau & un graphique.
