...
Une validation interne des questionnaires (coefficient \textalpha{} de \textsc{Cronbach}) sera réalisée sur le score \textsc{heiq} (validée pour ce score).
...
Le score sera présenté par les moyennes ± écart-type des huit thèmes pour chaque groupe & comparées par des tests t de \textsc{Student}. Une présentation graphique sera réalisée. 
...
Ce score d'auto-évaluation se compose de 11 items en échelle de Likert à cinq niveau. une présentation des résultats sera réalisée par un tabeau & un graphique (package `ggstats` [@ggstats]). Une comparaison du score global sera présentée. 
...
Une comparaison simple sera faite sur le nombre d'hospitalisation entre les deux groupes & présnetée par un Odd-Ratio. 
...
Un tableau présentera les résultats pour chaque item (nombre & pourcentage) groupe par groupe. Des comparaisons seront réalisées par un test t de \textsc{Student} avec correction de \textsc{Holm-Bonferroni} vu le grand nombre de tests que cela représente. 
