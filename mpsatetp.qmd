...
**Investigateur coordonnateur** : Anne-Charlotte TAUVERON -- Hôtel du Département du Val d’Oise – MDPH 
...
**Chef de projet** : Mme Mathilde WLODARCZY
...
items1 <- function(df = tt, deb, nlabs,expx = expx, classeur = classeur) {
nexp <- paste0(deb, "tab")
tab1 <- df |>
...
df |>
  ...
  add_p() |>
  bold_labels() |>
  pexptabph(exp = expx, nomfich = classeur, nomsheet = nexp)
}
```
...
tt |> 
  ...
  add_p() |>
   formt(oui = FALSE) |> 
  pexptabph(exp = expx, nomfich = classeur, nomsheet = "efr") |> scroll_box(width = "100%", height = "700px")
```
...
#| tab-cap: Score HEIQ - Thème HDA selon le groupe
...
#| tab-cap: Score HEIQ - Thème PAEL selon le groupe
...
```{r}
#| label: tbl-grheiq-ed
#| tab-cap: Score HEIQ - Thème ED selon le groupe
...
#| tab-cap: Score HEIQ - Thème SMI selon le groupe
...
#| tab-cap: Score HEIQ - Thème CAA selon le groupe
...
#| tab-cap: Score HEIQ - Thème STA selon le groupe
...
#| tab-cap: Score HEIQ - Thème SIS selon le groupe
...
#| tab-cap: Score HEIQ - Thème HSN selon le groupe
...
```{r}
#| label: tbl-had-anx
#| tab-cap: "Score d'anxiété (HAD) par groupe"
...
```{r}
#| label: tbl-had-dep
#| tab-cap: "Score de dépression (HAD) par groupe"
...
#| label: tbl-breq-2tot
#| tab-cap: Score BREQ 2 vs groupe
zz <- tt |> 
  dplyr::select(c(starts_with(deb))) |> 
  mutate_all(as.numeric)
zz$groupe <- tt$groupe
